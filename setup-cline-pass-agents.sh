...
merge_hermes_config() {
    ...
    if ! command -v ruby >/dev/null 2>&1; then
        echo "  WARN  ruby not found; skipping Hermes YAML merge" >&2
        return
    fi
...
exec "${local_bin}/cline-pass-key" "\${PI_CLINE_KEYCHAIN_SERVICE:-pi-cline-api-key}" "\${PI_CLINE_KEYCHAIN_ACCOUNT:-}"
EOF
...
exec "${local_bin}/cline-pass-key" "\${HERMES_CLINE_KEYCHAIN_SERVICE:-hermes-cline-api-key}" "\${HERMES_CLINE_KEYCHAIN_ACCOUNT:-}"
EOF
...
exec "${local_bin}/cline-pass-key" "\${OPENCODE_CLINE_KEYCHAIN_SERVICE:-opencode-cline-api-key}" "\${OPENCODE_CLINE_KEYCHAIN_ACCOUNT:-}"
EOF
}
...
for candidate in "\${HERMES_BIN:-}" "${TARGET_DIR}/.hermes/venvs/hermes-dev/bin/hermes" /opt/homebrew/bin/hermes /usr/local/bin/hermes /usr/bin/hermes; do
...
for candidate in "\${OPENCODE_BIN:-}" /opt/homebrew/bin/opencode /usr/local/bin/opencode /usr/bin/opencode; do
  [ -n "\$candidate" ] || continue
  [ -x "\$candidate" ] || continue
  exec "\$candidate" "\$@"
done
